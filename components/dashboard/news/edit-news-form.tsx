...
import { News, PublishStatus } from "@prisma/client"
...
import {  UPDATE_NEWS } from "@/actions/news.action"

interface Props {
    news: News
}

export const EditNewsForm = ({news}:Props) => {
    ...
    function onSubmit(values: z.infer<typeof NewsSchema>) {
        toast.loading("News updating...", {
            id: "update-news"
        })
        updateNews({values, id: news.id})
    }
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <Button type="submit" variant="ringHover" disabled={isPending}>Update</Button>
            </form>
        </Form>
    )
}